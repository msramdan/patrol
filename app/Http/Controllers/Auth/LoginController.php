...
class LoginController extends Controller
{
    ...
    */
    ...
    protected function login(Request $request)
    {
        ...
        if (!$user) {
            return $this->sendFailedLoginResponse($request, 'Email not found');
        }

        // Pengecekan status akun
        if ($user->is_active == 0) {
            return $this->sendFailedLoginResponse($request, 'Account is not active, Please contact the application admin');
        }
